...
In conclusion, the function names of LISP take more responsibilities to annotate operations than modern PLs. The more informations that function names show us, the better we can distinguish them and understand the purpose and usage of them.


@section[#:tag "retrofit-chinese-to-lisp"]{Chineseoid characters}
Compare to English,the writing of it is based on the pronounciation of its speaking, Chinese contrarily emphasize its writing, which has actually developed a way to write connected objects and concepts, we call it @hyperlink["https://en.wikipedia.org/wiki/Chinese_character_classification" "LiuShu(六书)"].

With the fact that chinese characters are just the implement of effectively using LiuShu for the communication of human, what I am doing here is using it to invent and design lots of chinese resembled characters(looks like chinese characters but can not be recognized by most Chinese people) for the concepts of Lisp and even programming language world to make human interact better with machines.
...
Further more, the connotations behind the characters work the same way, they are related to each other and complex concepts are broke to simple concepts, too.
